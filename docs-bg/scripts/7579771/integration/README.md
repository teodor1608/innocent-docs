...
Independent of `Config.Framework` — a QBCore server can still prefer ox\_lib-styled toasts (or vice versa), so pick whichever matches your server's look.
...
{% hint style="info" %}
Unlike the other bridges, `'auto'` falls back to `'ox'` here rather than `'custom'` if nothing else is detected — ox\_lib is already a hard dependency of this whole resource, so it's always a safe default.
{% endhint %}
